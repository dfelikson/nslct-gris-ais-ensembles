...
if ~getfieldvalue(options,'ensemblelaunch',false),
...
% If option 'ensembleprep' is specified, tar.gz the filelist and return
if getfieldvalue(options,'ensembleprep',false),
   compressstring=['tar -zcf ' md.private.runtimename '.tar.gz '];
   for i=1:numel(filelist),
      compressstring = [compressstring ' ' filelist{i}];
   end
   if cluster.interactive,
      compressstring = [compressstring ' ' md.private.runtimename '.run '  md.private.runtimename '.errlog ' md.private.runtimename '.outlog '];
   end
   system(compressstring);
   disp(['ensemble mode requested ... ' md.private.runtimename '.tar.gz created']);
   [flag, message, messageid] = movefile('*.toolkits', '~/.Trash','f');
   [flag, message, messageid] = movefile('*.queue', '~/.Trash','f');
   [flag, message, messageid] = movefile('*.bin', '~/.Trash','f');
   return;
elseif ~getfieldvalue(options,'ensemblelaunch',false),
   if isempty(restart),
      UploadQueueJob(cluster,md.miscellaneous.name,md.private.runtimename,filelist);
   end
end

% If option 'ensemblelaunch' is specified, upload all tar.gz files in one command
% and launch all jobs on the cluster (also in one command)
if getfieldvalue(options,'ensemblelaunch',false),
   ensemblejobs = getfieldvalue(options,'ensemblejobs',{});
   disp('ensemblelaunch mode requested:');
   disp(' uploading all jobs ...');
   issmscpout(cluster.name,md.cluster.executionpath,cluster.login,cluster.port,ensemblejobs);
   disp(' launching all jobs ...');
   launchcommand = '';
   for i=1:numel(ensemblejobs)
      disp(['  ' ensemblejobs{i}]);
      dirname = strrep(ensemblejobs{i}, '.tar.gz', '');
      modelname = strtok(ensemblejobs{i}, '-');;
      launchcommand=[launchcommand ' cd ' cluster.executionpath ' && rm -rf ./' dirname ' && mkdir ' dirname ...
                   ' && cd ' dirname ' && mv ../' dirname '.tar.gz ./ && tar -zxf ' dirname '.tar.gz  && sbatch ' modelname '.queue'];
      if i<numel(ensemblejobs)
